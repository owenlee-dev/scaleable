...
const CircularPicker = ({
  ...
  const pan = PanResponder.create({
    ...
    onPanResponderMove: (_, { moveX, moveY }) => {
      circle.current.measure((x, y, width, height, px, py) => {
         ...
         * @TODO
         */
        if ((newPos < -0.3 && pos > 1.3) || (newPos > 1.3 && pos < -0.3)) {
          return;
        }
        setPos(newPos);
        onChange(posToPercent(newPos));
  ...
  return (
    <Svg height={size} width={size} ref={circle}>
      ...
      <G
        transform={{
          translate: `${strokeWidth / 2 + radius + padding}, ${
            strokeWidth / 2 + radius + padding
          }`,
        }}
      >
        <Circle
      ...
      <G
        transform={{
          translate: `${center + padding}, ${strokeWidth / 2 + padding}`,
        }}
      >
      ...
      {steps &&
        steps.map((step, index) => (
          <G
            transform={{
              translate: `${step.x + padding}, ${step.y + padding}`,
            }}
            key={index}
          >
            <Circle
